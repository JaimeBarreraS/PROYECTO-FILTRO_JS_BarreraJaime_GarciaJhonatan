...
fetch(url)
    ...
    .then(Data => {
        ...
        <p>Type: <span>${Data[3].type}</span></p>
        <p>active: <span>${Data[3].active}</span></p>
        <p>Stages: <span>${Data[3].stages}</span></p>
        <p>Boosters: <span>${Data[3].boosters}</span></p>
        <p>Cost Per Launch: <span>$ ${Data[3].cost_per_launch}</span></p>
        <p>First Fling: <span>${Data[3].first_flight}</span></p>
        <p>Country: <span>${Data[3].country}</span></p>
        <p>Company: <span>${Data[3].company}</span></p>
        `;
    });
...
const spacexAPI = 'https://api.spacexdata.com/v4/rockets';
    ...
    async function fetchImages() {
        ...
        if (rockets.length > 0 && rockets[3].flickr_images.length > 0) {
            images = rockets[3].flickr_images;
            updateCarousel();
        }
    }
